      ...
      return this.events
        ...
        .reduce((output, event, idx, arr) => {
          if (isMulti(event) && !isStart(event, this.day.date)) {
            ...
            if (relative) {
              const occupied = arr[relative.order];

              if (occupied && relative.id !== occupied.id) {
                if (relative.order === occupied.order) {
                  event.order = occupied.order;
                  occupied.order = idx;
                }
              } else {
                event.order = relative.order;
              }

              // if (occupied && relative.id !== occupied.id) {
              //   if (relative.order === occupied.order) {
              //     event.order = occupied.order;
              //     occupied.order = relative.order - 1;
              //   }
              // } else {
              //   event.order = relative.order;
              // }
